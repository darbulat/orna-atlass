...
| Фоновые воркеры | Celery, RQ или Dramatiq с Redis broker | Обработка аудио, генерация waveform, извлечение метаданных, прогрев кеша |
...
- Режим списка для локаций и коллекций.
...
9. Worker обновляет `AudioSession.recording_status=ready`, если необходимые assets существуют.
...
- Refresh token хранится безопасно server-side или ротируется через auth provider.
- Role-based access control для admin/editor operations.
- Membership-based playback rules для защищенных sessions.
...
Чувствительные экологические локации могут требовать coordinate obfuscation. Некоторые protected habitats должны публично показывать approximate coordinates, сохраняя exact coordinates видимыми только администраторам.
...
- Atlas frontend shell с list fallback.
...
### Phase 5: Membership и protected playback
...
### Phase 6: Редакционная полировка
...
1. Для long-form playback начать с single files и добавить HLS позже при необходимости.
2. Для первого релиза использовать hybrid globe/map interface.
3. В качестве worker framework использовать RQ.
4. Для auth approach использовать hybrid.
5. Exact coordinates всегда публичны.
6. Memberships реализуются внутри системы.
