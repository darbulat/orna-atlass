...
Last verified: 2026-07-15. This document describes code that exists now. `ARCHITECTURE*.md` and `IMPLEMENTATION_PLAN_rus.md` may also describe target behavior; when they disagree, code, migrations and tests are authoritative.
...
| Frontend tests | Implemented baseline | Five Node-20 reducer/resource tests and twelve deterministic Playwright scenarios cover navigation, typed errors, membership, grant refresh, switch/abort races, seek, cleanup and keyboard/ARIA behavior. |
