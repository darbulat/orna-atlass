...
Last verified: 2026-07-19. This document describes code that exists now. `ARCHITECTURE*.md` and `IMPLEMENTATION_PLAN_rus.md` may also describe target behavior; when they disagree, code, migrations and tests are authoritative.
...
| Frontend tests and session listening UI | Implemented | Five Node-20 reducer/resource tests and thirty-two deterministic Playwright scenarios cover navigation, conversion flows, typed errors, membership, grant refresh, switch/abort races, seek, cleanup and keyboard/ARIA behavior. Public session details aggregate BirdNET detections by species, expose bounded filtered episode lists with seek-to-audio actions, omit absent recording metadata and keep pipeline assets collapsed behind technical details. |
