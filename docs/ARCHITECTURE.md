# ORNA Atlas backend architecture
...
The backend must preserve three product principles:
...
## 10. Dawn-line and local-time architecture
...
2. Choose worker framework: Celery, RQ, Dramatiq, or Arq.
3. Choose auth approach: custom JWT, managed auth provider, or hybrid.
4. Decide whether exact coordinates are always public or can be obfuscated by location.
5. Decide whether memberships are implemented internally or via Stripe Customer Portal integration.
