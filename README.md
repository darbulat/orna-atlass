...
| Support e-mail | `RESEND_API_KEY`, `RESEND_WEBHOOK_SECRET`, `SUPPORT_FORWARD_TO`, `SUPPORT_FROM_EMAIL` | Configure the first three together and set `SUPPORT_FORWARD_TO` to an operational owner other than `support@orna.land` to prevent recursive delivery. Resend posts signed `email.received` events to `/api/v1/support/webhooks/resend`; only mail addressed to `support@orna.land` is forwarded, attachment pagination and a 25 MiB cumulative attachment budget are fail-closed, and provider failures remain retryable. Keep webhook and API credentials out of version control. |
