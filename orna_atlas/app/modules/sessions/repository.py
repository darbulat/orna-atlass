...
async def replace_segment_bird_vocal_parts(
    session: AsyncSession,
    session_id: UUID,
    ...
    *,
    ...
    """Replace one segment without disturbing successful sibling segments."""
    await session.execute(
        delete(BirdVocalPart).where(
            BirdVocalPart.recording_segment_id == recording_segment_id,
            BirdVocalPart.analysis_provider == analysis_provider,
            BirdVocalPart.analysis_model_version == analysis_model_version,
        )
    )
